...
for i, linea in enumerate(lineas_entrada):
    if "CREATE LASTCHILD OF refError NAMESPACE '' NAME 'Code' VALUE 'GSS-415" in linea or "CREATE LASTCHILD OF refError NAMESPACE '' NAME 'Code' VALUE 'GSS-422" in linea:
        for j in range(max(0, i - 20), i):
            if "SET Error = Not" in lineas_entrada[j]:
                linea_salida = lineas_entrada[j].split("Executing statement")[1].strip()
